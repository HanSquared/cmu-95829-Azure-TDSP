...
We can see we have 21 columns and attributes with 7,043 records. The median of monthly charges by customer is $70.35 meanwhile the Total charges $1,397.The average tenure is 32 months while 1,869 customers have left the company this quarter.
```{r summary}
...
```
We have monthly charges, total charges and tenure as numerical values. The others are factor or categorical values.
...
We also need to convert senior citizen to a factor
```{r senior citizen}
...
Checking outliers for monthly charges

```{r outliers}
# Validate if there are outliers in monthly charges
...
In the histogram, We see tenure is left skewed with churn customers, this means that they trend to leave the company within 12 months.  On the other hand, the bar graph shows the average monthly charges in Churn customers was higher than none churn customers. 
...
Gender by churn is not a relevant attribute considering the proportion of churn is similar among male and female.
...
Churn is more common in customers that pay in a monthly basis. Having a more long term contract with customers could lead to more retention.
```{r contract}
...
Customers with fiber optic has more chances to churn.
```{r internetservice}
...
Customer that pay via transfer or credit/debit card has less churn rate.
```{r payment}
...
Additionally, we can see customers with dependents and partner are less likely to churn.
...
Now, let's graph correlation of numerical values:
```{r correlation}
Churn_file %>% 
...
We see a strong positive linear relationship between the tenure and total charges of 0.8. The boxplots show significant variance in tenure.